...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # d_k = d_model / h
    # query: (bs, h, seq_len, d_k)
    # key:   (bs, h, seq_len, d_k)
    # value: (bs, h, seq-len, d_v)
    # e:     (bs, h, seq-len, seq_len)
    # v_dot: (bs, h, seq_len, d_v)
    # out:   (bs, seq_len, d_model)

    bs, h, seq_len, d_k = query.shape

    qk_dot = torch.einsum("...sk,...jk->...sj", query, key) / self.attention_head_size**0.5
    qk_dot = qk_dot + attention_mask
    e      = F.softmax(qk_dot, dim=-1)
    e      = self.dropout(e)
    v_dot  = torch.einsum("...sj,...jv->...sv", e, value)
    v_dot  = v_dot.transpose(1, 2)
    out    = v_dot.reshape((bs, seq_len, -1))
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    x = self.add_norm(x, y, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    y = self.interm_af(self.interm_dense(x))
    x = self.add_norm(x, y, self.out_dense, self.out_dropout, self.out_layer_norm)
