...
def model_eval_lin(lin_dataloader, model, device):
    model.eval()
    lin_y_true = []
    lin_y_pred = []
    lin_sent_ids = []
    for step, batch in enumerate(tqdm(lin_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        ...
        lin_y_pred.extend(y_hat)
        lin_y_true.extend(b_labels)
        lin_sent_ids.extend(b_sent_ids)

    linguistic_accuracy = np.mean(np.array(lin_y_pred) == np.array(lin_y_true))

    return linguistic_accuracy,lin_y_pred, lin_sent_ids
...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        # Evaluate linguistic acceptability classification.
        linguistic_accuracy, lin_y_pred, lin_sent_ids = model_eval_lin(lin_dataloader, model, device)
        ...
        print(f'Linguistic classification accuracy: {linguistic_accuracy:.3f}')

        return (sentiment_accuracy,sst_y_pred, sst_sent_ids,
                paraphrase_accuracy, para_y_pred, para_sent_ids,
                sts_corr, sts_y_pred, sts_sent_ids,
                linguistic_accuracy,lin_y_pred,lin_sent_ids)
...
def model_eval_test_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            (b_ids, b_mask,
             b_sent_ids) = (batch['token_ids'], batch['attention_mask'],
                          batch['sent_ids'])
        ...
        # Evaluate linguistic classification.
        lin_y_pred = []
        lin_sent_ids = []
        for step, batch in enumerate(tqdm(lin_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            b_ids, b_mask, b_sent_ids = batch['token_ids'], batch['attention_mask'],  batch['sent_ids']
